...
import UpdateViews from "@/components/pages/blogs/UpdateViews";

async function getBlogData(id: string) {
  try {
    const response = await fetch(`http://localhost:3000/api/public/blogs?id=${id}`, {
      next: { revalidate: 60 }
    });
...
export default async function BlogPage({ params }: { params: { id: string } }) {
  const blog: BlogDocument = await getBlogData(params.id);
  ...
  return (
    <main className="w-full light:bg-white">
      {/* Client-side component for view tracking */}
      <UpdateViews blogId={params.id} />
