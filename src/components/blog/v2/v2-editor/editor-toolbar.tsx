...
export function EditorToolbar({ editor }: EditorToolbarProps) {
  ...
  return (
    <div className="dark:bg-slate-900 bg-gray-200 border-b border-border p-2 flex flex-wrap gap-1 items-center rounded-t-lg dark:border-cyan-400">
      {/* Text Formatting */}
        ...
        <Select value={codeLanguage} onValueChange={setCodeLanguage}>
          ...
          <SelectContent className="max-h-80 overflow-y-auto">
            ...
            {Object.entries(groupedLanguages).map(([category, languages]) => (
              <div key={category}>
                <div className="px-2 py-1 text-sm font-semibold text-muted-foreground tracking-wide border-t border-b border-black w-full text-center my-2 dark:bg-slate-950 dark:text-cyan-400 dark:border-cyan-400">
                  {category}
                </div>
                {languages.map((lang) => (
      ...
      <div className="flex items-center gap-1">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => editor.chain().focus().setDivider().createParagraphNear().run()}
          title="Add Divider"
        >
